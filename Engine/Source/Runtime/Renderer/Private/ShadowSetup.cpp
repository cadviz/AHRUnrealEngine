...
/** Initialization constructor for a whole-scene shadow. e.g. reflective shadow map, directional light cascade or point light */
FProjectedShadowInfo::FProjectedShadowInfo(
	FLightSceneInfo* InLightSceneInfo,
					...
					if (!bWholeSceneDirectionalShadow)
					{
						...
						for (int32 MeshIndex = 0; MeshIndex < PrimitiveSceneInfo->StaticMeshes.Num(); MeshIndex++)
						{
							...
							if (StaticMesh.CastShadow)
							{
								...
								if(((!IsTranslucentBlendMode(BlendMode)) && ShadingModel != MSM_Unlit) || (bReflectiveShadowmap && Material->ShouldInjectEmissiveIntoDynamicGI())) 
								{
